...
if __name__ == "__main__":
    ...
    for epoch in range(num_epochs):
        ...
        for inputs, labels in dataloader:
            ...
            # Upsample output to match label size
            final_output = F.interpolate(final_output, size=labels.shape[2:], mode='bilinear', align_corners=False)
    ...
    torch.save(model.state_dict(), "unet_model_out_original_res.pth")
    print("\nModel saved as 'unet_model_out_original_res.pth'.")

    # Load the trained model
    model.load_state_dict(torch.load("unet_model_out_original_res.pth"))
