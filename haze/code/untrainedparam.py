...
class Unet(nn.Module):
    def __init__(self):
        ...
        self.final_upsample = nn.ConvTranspose2d(256, 1, kernel_size=2, stride=2)  # Final upsampling to match 256x256
    ...
    def forward(self, x):
        ...
        final_output = self.final_upsample(fc_output)  # Upsample to 256x256 and reduce channels to 1
        return final_output
...
if __name__ == "__main__":
    ...
    # Use the model with untrained parameters
    model = Unet()  # Create a new instance of the model with untrained parameters
    model.eval()  # Set the model to evaluation mode
    print("\nUsing untrained model parameters for prediction.")

    # Load and preprocess the hazy image
    hazy_image_path = r"C:\Users\oishi\Documents\haze\haze\resized_input_images\resized_input_images\3138.jpg"
    ...
    predicted_map_np = predicted_map.squeeze(0).squeeze(0).numpy()  # Remove batch and channel dimensions
    ...
    # Plot the predicted transmission map
    plt.figure(figsize=(6, 6))
    plt.title("Predicted Transmission Map (Untrained Parameters)")
    plt.imshow(predicted_map_np_255, cmap="gray")
    plt.axis("off")
    plt.show()

    # Save the predicted transmission map
    #predicted_map_image = Image.fromarray(predicted_map_np_255)
    #predicted_map_image.save("predicted_transmission_map_untrained.jpg")
    #print("\nPredicted transmission map saved as 'predicted_transmission_map_untrained.jpg'.")